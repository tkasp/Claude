...
export function useBowtieLayout(bowtie: Bowtie | undefined): { nodes: Node[]; edges: Edge[] } {
  return useMemo(() => {
    ...
    const lastBarrierRight = FIRST_BARRIER_X + Math.max(maxBarriers - 1, 0) * BARRIER_STEP + NODE_WIDTH
    ...
    const lastMitigationRight = FIRST_MITIGATION_X + Math.max(maxMitigations - 1, 0) * BARRIER_STEP + NODE_WIDTH
    const CONSEQUENCE_X = lastMitigationRight + CENTER_GAP

    const hazardNodeId = `hazard-${bowtie.id}`
    nodes.push({
      id: hazardNodeId, type: 'hazard',
      position: { x: CENTER_X - 10, y: HAZARD_Y },
      data: { bowtieId: bowtie.id, hazard: bowtie.hazard },
      width: NODE_WIDTH + 20, height: 60, draggable: false
    })

    nodes.push({
      id: bowtie.topEvent.id, type: 'topEvent',
      position: { x: CENTER_X, y: TOP_EVENT_Y },
      data: { bowtieId: bowtie.id, label: bowtie.topEvent.label },
      width: TOP_EVENT_SIZE, height: TOP_EVENT_SIZE, draggable: false
    })

    edges.push({
      id: `e-hazard-${bowtie.id}`, source: hazardNodeId, target: bowtie.topEvent.id,
      type: 'straight', style: edgeStyle, sourceHandle: 'bottom', targetHandle: 'top'
    })
    ...
    bowtie.causes.forEach((cause, ci) => {
      ...
      cause.barriers.forEach((barrier, bi) => {
        ...
        nodes.push({
          id: barrier.id, type: 'barrier',
          position: { x: bx, y: rowY - NODE_HEIGHT / 2 },
          data: {
            bowtieId: bowtie.id, causeId: cause.id, barrierId: barrier.id,
            label: barrier.label, effectiveness: barrier.effectiveness,
            isSECE: barrier.isSECE, seceId: barrier.seceId,
            actionCount: barrier.actions?.length ?? 0
          },
          width: NODE_WIDTH, height: NODE_HEIGHT, draggable: false
        })
        edges.push({
          id: `e-${prevId}-${barrier.id}`, source: prevId, target: barrier.id,
          type: 'smoothstep', style: edgeStyle, sourceHandle: 'right', targetHandle: 'left'
        })
        prevId = barrier.id
      })

      edges.push({
        id: `e-${prevId}-te-${ci}`, source: prevId, target: bowtie.topEvent.id,
        type: 'smoothstep', style: edgeStyle, sourceHandle: 'right', targetHandle: 'left'
      })
    })
    ...
    bowtie.consequences.forEach((con, ci) => {
      ...
      nodes.push({
        id: con.id, type: 'consequence',
        position: { x: CONSEQUENCE_X, y: rowY - NODE_HEIGHT / 2 },
        data: { bowtieId: bowtie.id, consequenceId: con.id, label: con.label, severity: con.severity },
        width: NODE_WIDTH, height: NODE_HEIGHT + 22, draggable: true
      })
      ...
      con.mitigations.forEach((mit, mi) => {
        ...
        nodes.push({
          id: mit.id, type: 'mitigation',
          position: { x: px, y: rowY - NODE_HEIGHT / 2 },
          data: {
            bowtieId: bowtie.id, consequenceId: con.id, mitigationId: mit.id,
            label: mit.label, effectiveness: mit.effectiveness,
            isSECE: mit.isSECE, seceId: mit.seceId,
            actionCount: mit.actions?.length ?? 0
          },
          width: NODE_WIDTH, height: NODE_HEIGHT, draggable: false
        })
        edges.push({
          id: `e-${prevId}-${mit.id}`, source: prevId, target: mit.id,
          type: 'smoothstep', style: edgeStyle, sourceHandle: 'right', targetHandle: 'left'
        })
        prevId = mit.id
      })
      ...
      edges.push({
        id: `e-${prevId}-con-${ci}`, source: prevId, target: con.id,
        type: 'smoothstep', style: edgeStyle, sourceHandle: 'right', targetHandle: 'left'
      })
    })
