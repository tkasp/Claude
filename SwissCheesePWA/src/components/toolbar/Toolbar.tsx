...
export function Toolbar({ onNewProject, onOpenProject }: ToolbarProps): React.ReactElement {
  ...
  const handleSave = async (): Promise<void> => {
    ...
    const result = await saveProject(JSON.stringify(project), project.id)
    if (result.success) {
      if (result.fileName) store.setProjectFilePath(project.id, result.fileName)
      flash('Project saved')
    }
  }
  ...
  const handleExportPng = async (): Promise<void> => {
    ...
    try {
      ...
      exportPng(dataUrl, bowtie?.name ?? 'bowtie')
      flash('PNG exported')
    } catch (e) {
      console.error(e)
