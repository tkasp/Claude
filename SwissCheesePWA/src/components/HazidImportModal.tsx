...
export function HazidImportModal({ projectId, attachment, onClose }: Props): React.ReactElement {
  ...
  const toggleAllFiltered = (): void => {
    if (allFilteredSelected) {
      setSelected((prev) => { const next = new Set(prev); filteredRows.forEach((r) => next.delete(r.rowIndex)); return next })
    } else {
      setSelected((prev) => { const next = new Set(prev); filteredRows.forEach((r) => next.add(r.rowIndex)); return next })
    }
  }
  ...
  const handleHighSevToggle = (): void => {
    ...
    setHighSevOnly(next)
    if (next && parsed) {
      const highRows = new Set(parsed.rows.filter((r) => isHighSeverity(getCell(r, 'riskRating'))).map((r) => r.rowIndex))
  ...
  const handleGenerate = (): void => {
    ...
    const stubs = rows.map((row, i) => {
      ...
      const topEvent = getCell(row, 'topEvent')
      const name = hazardId ? `${hazardId} – ${hazardName || topEvent || 'Bowtie'}` : hazardName || `Bowtie ${i + 1}`
  ...
  const FilterInput = ({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder: string }): React.ReactElement => (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-slate-800 rounded-lg shadow-2xl flex flex-col w-[950px] max-w-[95vw] max-h-[88vh] text-slate-100">
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          {loading && (
            <div className="flex-1 flex items-center justify-center gap-3 text-slate-300">
              <Loader size={20} className="animate-spin" />Parsing HAZID file…
            </div>
          )}
          {error && (
            <div className="flex-1 flex flex-col items-center justify-center gap-3 text-red-400 p-8">
              <AlertCircle size={32} /><div className="text-sm text-center">{error}</div>
            </div>
          )}
            ...
            <>
              <div className="px-4 py-2.5 border-b border-slate-700 bg-slate-900/50">
                <div className="flex items-center gap-3 mb-2">
                  <div className="flex items-center gap-1.5 text-xs text-slate-400 font-semibold uppercase tracking-wide"><Filter size={12} /> Filters</div>
                  <button onClick={handleHighSevToggle} className={`px-3 py-1 rounded-full text-xs font-bold border transition-colors ${highSevOnly ? 'bg-red-700 border-red-500 text-white' : 'border-slate-600 text-slate-400 hover:border-red-500 hover:text-red-400'}`}>
                    Severity 4 & 5 Only
                  </button>
                  {(filterHazardId || filterHazard || filterTopEvent || filterRisk) && (
                    <button onClick={() => { setFilterHazardId(''); setFilterHazard(''); setFilterTopEvent(''); setFilterRisk('') }} className="text-xs text-slate-500 hover:text-slate-300">Clear filters</button>
                  )}
                  <span className="ml-auto text-xs text-slate-500">Showing {filteredRows.length} / {parsed.rows.length} rows</span>
                </div>
              ...
              <div className="flex-1 overflow-auto">
                <table className="w-full text-xs border-collapse">
                  <thead className="sticky top-0 bg-slate-700 z-10">
                    <tr>
                      <th className="px-3 py-2 text-left border-b border-slate-600 w-8"><input type="checkbox" checked={allFilteredSelected} onChange={toggleAllFiltered} /></th>
                  ...
                  <tbody>
                    {filteredRows.map((row) => {
                      ...
                      return (
                        <tr key={row.rowIndex} className={`cursor-pointer border-b border-slate-700/60 ${selected.has(row.rowIndex) ? 'bg-blue-900/40 hover:bg-blue-900/60' : 'hover:bg-slate-700/40'}`} onClick={() => toggleRow(row.rowIndex)}>
                          <td className="px-3 py-1.5"><input type="checkbox" checked={selected.has(row.rowIndex)} onChange={() => toggleRow(row.rowIndex)} onClick={(e) => e.stopPropagation()} /></td>
                          ...
                          <td className="px-3 py-1.5 whitespace-nowrap"><span className={`font-semibold ${high ? 'text-red-400' : 'text-slate-400'}`}>{risk || '—'}</span></td>
                        </tr>
                      )
                    })}
                    {filteredRows.length === 0 && (
                      <tr><td colSpan={5} className="px-5 py-8 text-center text-slate-500">No rows match the current filters.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-slate-700 bg-slate-900/40">
          <div className="text-xs text-slate-400">{parsed ? `${selected.size} row${selected.size !== 1 ? 's' : ''} selected` : ''}</div>
          <div className="flex gap-2">
            ...
            <button onClick={handleGenerate} disabled={selected.size === 0 || !parsed} className="px-4 py-1.5 text-sm rounded bg-blue-600 text-white font-semibold hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed">
