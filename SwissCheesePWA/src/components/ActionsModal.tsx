...
export function ActionsModal({ projectId, target, onClose }: Props): React.ReactElement | null {
  ...
  // Subscribe to the projects array so the modal re-renders on edits.
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-2xl flex flex-col w-[680px] max-w-[95vw] max-h-[85vh]">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200">
          <div>
            <div className="font-semibold text-gray-900">
              Actions — {target.kind === 'barrier' ? 'Barrier' : 'Mitigation'}
            </div>
                  ...
                  <tr key={a.id} className="border-t border-gray-100 align-top">
                    ...
                    <td className="px-2 py-2">
                      <textarea
                        ...
                        value={a.text}
                        onChange={(e) =>
                          store.updateAction(projectId, target, a.id, { text: e.target.value })
                        }
                      />
                    </td>
                    <td className="px-2 py-2">
                      <input
                        ...
                        value={a.dueDate}
                        onChange={(e) =>
                          store.updateAction(projectId, target, a.id, { dueDate: e.target.value })
                        }
                      />
                    </td>
                    <td className="px-2 py-2">
                      <button
                        title="Delete action"
                        onClick={() => store.deleteAction(projectId, target, a.id)}
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-gray-200 bg-gray-50">
          <button
            onClick={() => store.addAction(projectId, target)}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded bg-violet-600 text-white font-semibold hover:bg-violet-700"
          >
