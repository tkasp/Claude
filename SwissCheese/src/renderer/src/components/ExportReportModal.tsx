...
export function ExportReportModal({ project, onClose }: Props): React.ReactElement {
  ...
  const [errorMsg, setErrorMsg] = useState('')
    // Track which bowtie we want the canvas to render (for sequential capture)
  ...
  // Poll for the capturer to be registered after switching to a bowtie.
  useEffect(() => {
  ...
  const captureBowtie = (bowtieId: string): Promise<string | null> => {
    return new Promise((resolve) => {
      ...
      // Switch the active view to this bowtie so its canvas mounts.
  ...
  const handleStart = async (): Promise<void> => {
    ...
    try {
      ...
      const result = await window.electronAPI.savePdf(base64, project.name)
      if (result.success) {
        setPhase('done')
      } else {
        setPhase('idle')
        ...
        <div className="p-5">
          ...
          {phase === 'done' && (
            <div className="flex flex-col items-center gap-3 py-4">
              ...
              <button
                onClick={onClose}
                className="mt-2 px-5 py-2 rounded bg-slate-700 text-slate-200 text-sm hover:bg-slate-600"
              >
                Close
              </button>
            </div>
