...
// Rows with risk rating starting with "4" or "5" are high-severity.
...
export function HazidImportModal({ projectId, attachment, onClose }: Props): React.ReactElement {
  ...
  const handleHighSevToggle = (): void => {
    ...
    setHighSevOnly(next)
    if (next && parsed) {
      // Auto-select high severity rows, deselect others
      ...
      <div className="bg-slate-800 rounded-lg shadow-2xl flex flex-col w-[950px] max-w-[95vw] max-h-[88vh] text-slate-100">
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-1.5 text-slate-200 max-w-[200px]">
                            {getCell(row, 'topEvent') || '—'}
                          </td>
                          <td className="px-3 py-1.5 whitespace-nowrap">
                            <span
                              className={`font-semibold ${high ? 'text-red-400' : 'text-slate-400'}`}
                            >
                              {risk || '—'}
                            </span>
                          </td>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-slate-700 bg-slate-900/40">
          ...
          <div className="flex gap-2">
            <button
              onClick={onClose}
              className="px-4 py-1.5 text-sm rounded border border-slate-600 text-slate-300 hover:bg-slate-700"
            >
              Cancel
            </button>
            <button
